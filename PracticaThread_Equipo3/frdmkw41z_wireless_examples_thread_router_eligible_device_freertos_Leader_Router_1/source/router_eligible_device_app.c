...
#define APP_EQUIPO3_COUNTER_MAX_c               200
#define APP_EQUIPO3_TIMER_PERIOD_c              1000    /* milliseconds */
...
/* Accelerometer handle and I2C master handle */
fxos_handle_t gFxosHandle;
i2c_master_handle_t gAccelMasterHandle;
/* FXOS device candidate addresses */
static const uint8_t mAccelAddress[] = {0x1CU, 0x1DU, 0x1EU, 0x1FU};
...
)
{
    ...
    switch(pEventParams->code)
    {
        ...
        case gThrEv_GeneralInd_Connected_c:
            ...
            /* Start counter timer (1 second) } */
            if(mEquipo3TimerId == gTmrInvalidTimerID_c)
            {
            	mEquipo3TimerId = TMR_AllocateTimer();
            }
            if(mEquipo3TimerId != gTmrInvalidTimerID_c)
            {
            	gEquipo3Counter = APP_EQUIPO3_COUNTER_MIN_c;
            	TMR_StartIntervalTimer(mEquipo3TimerId, APP_EQUIPO3_TIMER_PERIOD_c,
...
/*!*************************************************************************************************
\brief  Timer ISR-context callback that posts a counter-tick message to the application queue.
...
\brief  Initializes the accelerometer over I2C.
        Scans the candidate I2C addresses, calls FXOS_Init(), and stores the
...
)
{
    ...
    uint8_t i;
    uint8_t regResult = 0;
    uint8_t arrayAddrSize = sizeof(mAccelAddress) / sizeof(mAccelAddress[0]);
    ...
    gFxosHandle.i2cHandle = &gAccelMasterHandle;

    /* Initialize I2C master */
    I2C_MasterGetDefaultConfig(&i2cConfig);
    I2C_MasterInit(BOARD_ACCEL_I2C_BASEADDR, &i2cConfig, i2cSourceClock);
    I2C_MasterTransferCreateHandle(BOARD_ACCEL_I2C_BASEADDR, &gAccelMasterHandle, NULL, NULL);

    /* Scan candidate addresses to find the sensor */
    for(i = 0; i < arrayAddrSize; i++)
    {
        gFxosHandle.xfer.slaveAddress = mAccelAddress[i];
...
)
{
	...
	if(FXOS_ReadSensorData(&gFxosHandle, &sensorData) == kStatus_Success)
	{
		...
		xData = (int16_t)((uint16_t)((uint16_t)sensorData.accelXMSB << 8) | sensorData.accelXLSB) / 4U;
		yData = (int16_t)((uint16_t)((uint16_t)sensorData.accelYMSB << 8) | sensorData.accelYLSB) / 4U;
		zData = (int16_t)((uint16_t)((uint16_t)sensorData.accelZMSB << 8) | sensorData.accelZLSB) / 4U;

		/* Format response string */
		accelStrLen = snprintf(accelStr, sizeof(accelStr), "x=%d y=%d z=%d", xData, yData, zData);
	}
	else
	{
		accelStrLen = snprintf(accelStr, sizeof(accelStr), "accel read error");
	}

    /* Send response with accel data for both CON and NON GET */
    if(isConfirmable)
    {
        COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, (uint8_t *)accelStr, accelStrLen);
    }
    else
    {
        COAP_Send(pSession, gCoapMsgTypeNonPost_c, (uint8_t *)accelStr, accelStrLen);
    }
